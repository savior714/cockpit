...
2. 최근 변경 이력 추적 및 궤적 증거 패스 (CHANGE TRACE PASS):
   ...
   - 권장 증거 패턴:
     ...
     (6) 최근 궤적과 객관적 상태가 충분히 설명되면 멈춤 (전체 Git 이력을 기계적으로 재현하지 않음)
...
7. 작성 (WRITE): 아래 마크다운 구조와 원칙에 맞춰 사실 기반으로 작성하되, 기술 증거는 점진적 공개(근거 섹션 또는 부차적 설명)로 배치한다.
   ...
   - `## 최근 진척`: 단순 커밋/태스크 로그가 아닌, "이전 상태에서 현재 상태로 어떻게 실질적으로 도달했는가"를 설명하는 시맨틱 전환 이력(`실질적 변경 → 프로젝트 역량/상태 영향`)을 최신순으로 작성한다 (증거 기반 약 5~8개 항목 권장, 억지 쿼터 채우기 금지).
   - `## 제품 목표`: 프로젝트의 존재 이유, 해결하려는 실질적 필요, 성공의 경계를 정의하는 정보 밀도 높은 문장으로 작성한다.
   - `## 확정된 방향`: 현재 해석 및 향후 작업을 실질적으로 제약하는 영속적 결정들만 선별 기록한다.
...
## 최근 진척
- **[주요 시맨틱 전환 1]** — 실질적 변경 내용 및 프로젝트 상태/역량/방향에 미친 영향 (최신순 권장, 약 5~8개 항목)
- **[주요 시맨틱 전환 2]** — ...
...
1. **현재 상태 (Current State — 최우선 독서면)**:
   ...
   - **영역 상세 (`## 영역 상세`)**: 각 영역의 의미, 현재 수준, 남은 문제, 근거.
