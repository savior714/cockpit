...
| `## 영역 상세` / `## Area Details` | 특정 영역의 의미·현재 수준·근거는 무엇인가? | **필수.** 지도 항목마다 `### 제목` + `#### 의미` / `#### 현재 수준` / `#### 근거` 권장. `의미`는 무엇을 하는 기능인지 실제 제품 행동 한 문장으로, `현재 수준`은 실제로 구현·검증된 사실 한두 문장으로 쓴다. "구현됨·부분 완료" 같은 평가만 쓰지 않고 실제 가능한 행동/state를 말한다. 추상 성숙도 표현 단독(Strong·Partial식)이나 해독을 요구하는 내부 ontology 표현은 쓰지 않는다. `남은 문제`는 아직 무엇이 연결·검증되지 않았는지 구체적 제품 사실로 쓴다. 긴 기술 설명·측정·proof는 `근거`에 둔다. 지도 한 줄 설명과 같은 문장을 반복하지 않는다. 증거로 확인된 미해결 문제가 있을 때만 `#### 남은 문제`를 둔다. 없으면 섹션 자체를 생략한다. 닫힌 범위가 다시 검토되어야 하는 조건이 있을 때만 `#### 다시 열리는 조건`을 두고, 없으면 생략한다. Cockpit은 heading 구조만 따라 근거/남은 문제 표시를 정하며 본문 wording으로 검증 상태·open/closed를 재판정하지 않는다. |
