...
|---|---|---|
| `## 프로젝트 지도` / `## Project Map` | 프로젝트는 어떤 주요 부분으로 이루어져 있는가? 지금 어디쯤인가? | **필수.** `###` 레일 → `####` 그룹 → 목록 항목. 항목은 `- **제목** — 한 줄 설명`. 제목은 영역 상세의 `### 제목`과 정확히 일치해야 한다. 제목만 보고 실제 기능·업무 영역을 알 수 있게 쉬운 우리말 제품 언어로 쓴다. 한 줄 설명은 무엇을 하는 기능인지 실제 제품 행동으로 쓴다. 내부 설계 용어가 표현을 지배하지 않게 하고 실제 대상을 숨기는 압축 명사구는 쓰지 않는다. 레일·그룹 이름은 프로젝트 고유 어휘를 그대로 쓴다. `#### 현재 단계`(또는 `Current Stage`)는 정확한 문자열이며 선택 사항인 YOU ARE HERE 위치 표시다. 문서 전체에서 최대 1개다. 하나의 그룹 안에는 현재 관련 영역 여러 개가 있어도 된다. 그 외 그룹에는 Cockpit 특권이 없다. 순서 목록은 순서로, 글머리 목록은 동등 항목으로 읽힌다. |
| `## 영역 상세` / `## Area Details` | 특정 영역의 의미·현재 수준·근거는 무엇인가? | **필수.** 지도 항목마다 `### 제목` + `#### 의미` / `#### 현재 수준` / `#### 근거` 권장. `의미`는 무엇을 하는 기능인지 실제 제품 행동 한 문장으로, `현재 수준`은 실제로 구현·검증된 사실 한두 문장으로 쓴다. 추상 성숙도 표현 단독(Strong·Partial식)이나 해독을 요구하는 내부 ontology 표현은 쓰지 않는다. `남은 문제`는 아직 무엇이 연결·검증되지 않았는지 구체적 제품 사실로 쓴다. 긴 기술 설명·측정·proof는 `근거`에 둔다. 지도 한 줄 설명과 같은 문장을 반복하지 않는다. 증거로 확인된 미해결 문제가 있을 때만 `#### 남은 문제`를 둔다. 없으면 섹션 자체를 생략한다. Cockpit은 heading 구조만 따라 근거/남은 문제 표시를 정하며 본문 wording으로 검증 상태·open/closed를 재판정하지 않는다. |
| `## 현재 상황` / `## Current Situation` | 프로젝트 전체가 지금 어떤 상태인가? | project-level 압축 서술. 현재 어디까지 왔는지·무엇이 이미 확보됐는지·지금 가장 중요한 미완료 전환이 무엇인지 2~4문장으로 쓰고 배경·이유·해설·이력은 영역 상세와 근거에 둔다. 개별 작업 chronology가 아니다. |
| `## 다음 전환` / `## Next Transition` | 다음에 어떤 상태로 넘어가려 하는가? | `A 상태 → B 상태` + 한 절의 완료 조건. 이후 여정이나 상세 판정 기준은 지도와 영역 상세에 둔다. 실행 command가 아니다. |
| `## 직면한 문제` / `## Facing Issues` | 전환을 막거나 제한하는 것이 있는가? | **선택.** 실제로 제한하는 blocker/제약이 있을 때만 짧게 쓰고, 없으면 섹션 자체를 생략한다. 영역 `남은 문제`와 같은 원인을 그대로 복사하지 않고 전환 제한 관점에서만 쓴다. |
...
4. **개요는 얕게, 근거는 깊게**: commit SHA, 파일 경로, test command 같은 저수준 증거를 `현재 상황`/`다음 전환`/`직면한 문제`에 복사하지 않는다. 그런 증거는 영역 상세와 handoff에 둔다. 이는 작성 권장이며 구조적 실패가 아니다.
...
9. **구체적 제품 언어로**: 카드 제목·설명·현재 수준·남은 문제는 저장소를 모르는 독자가 제목+설명만으로 이해할 수 있게 실제 대상·행동·결과로 쓴다. 추상 ontology 해독을 요구하는 표현과 실제 대상을 숨기는 압축 명사구는 쓰지 않는다. 입증되지 않은 상태를 더 강하게 주장하지 않는다.
