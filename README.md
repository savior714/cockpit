...
6. 정보 계층 분리와 의미 우선 언어 (Information Hierarchy & Meaning-First):
   - 1차 독서면 (First-Reading / Human-Facing): 현재 집중(Current Focus), 지도 제목, 지도 한 줄 요약, '의미', '현재 수준' 도입부, (존재할 경우) '남은 문제'.
...
8. 영역 상세 작성 계약 (Area Detail Contract):
   - **필수 기술 섹션 (Required Evidence-Bearing Sections)**:
     * `#### 의미`: 외부인에게 해당 영역의 실질적 역할과 경계를 설명.
     * `#### 현재 수준`: 인간이 이해 가능한 수준/상태를 먼저 서술하고 한계 명시.
     * `#### 근거`: 코드 경로, 테스트, 런타임 결과, 커밋 등 가장 구체적인 구현 증거 배치.
   - **선택적 문제/조건 섹션 (Optional Evidence-Admitted Sections)**:
     * `#### 남은 문제`: 신선한 저장소/런타임/SSOT 증거로 뒷받침되는 구체적 미해결 문제가 있을 때만 작성 (증거 없는 추측/필러/플레이스홀더 금지, 없으면 섹션 자체 생략).
...
독자 관점의 필수 항목(의미, 현재 수준, 근거)과 명시적 관계 문법을 명확히 유지하고,
