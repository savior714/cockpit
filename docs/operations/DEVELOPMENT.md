...
For an ordinary bounded tracked-repository mutation task, successful closure includes ordinary safe non-force fast-forward publication directly to canonical `main` plus remote read-back, unless the work order explicitly restricts the task to local-only work (`LOCAL_ONLY`, `NO_PUSH`, `CANDIDATE_ONLY`, do-not-publish, or equivalent). Test/verify/review alone authorizes no source mutation. Repository publication is distinct from deployment/provider/production side effects; neither authority is inferred from the other. Never force-push or rewrite history; destructive/history-rewriting Git and separate external side effects still require their own explicit authority.

For ordinary publication under this default, when no local-only restriction exists:
