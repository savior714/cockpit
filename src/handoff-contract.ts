...
function formatProjectModelAdmissionLines(scope: "focus" | "area" = "focus"): string[] {
  ...
  return [
    ...
    "   - [Canonical vs Proposed Authority — repository tasks only] repository authority가 실제로 관여하는 task에서만 evidence class를 분리하라: `CANONICAL_AUTHORITY`는 fresh published/tracked authoritative ref에서 admit한 contract이고, 실제 현재 상태/적용 여부는 fresh current runtime/repository evidence로 판정한다. `LOCAL_TRACKED_SNAPSHOT`은 현재 `HEAD`와 tracked files를 담은 local snapshot이며 canonical ref와 identity를 직접 대조하기 전에는 canonical authority로 취급하지 않는다. `PROPOSED_WORKTREE_SEMANTICS`는 dirty/untracked working-tree candidate다. 후자는 investigation evidence로는 사용할 수 있지만 canonical authority로 부르거나 승격하지 않으며, canonical과 충돌하면 그 mismatch 자체를 조사하라.",
