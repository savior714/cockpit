...
export const MATURITY_STATE_ALIASES: Record<string, MaturityState> = {
  STRONG: "STRONG",
  "강함": "STRONG",
  "강한": "STRONG",
  PARTIAL: "PARTIAL",
...
export const FRONTIER_SUBSECTION_ALIASES: string[][] = [
  ["왜 지금", "why now"],
...
export function isSemanticMetadataLine(line: string): boolean {
...
export function firstSemanticSentence(value: string): string {
  const lines = value
    .split(/\r?\n/)
...
export function parseMaturityState(value: string): MaturityState | null {
  const clean = stripInlineMarkup(value).trim().toUpperCase();
  return MATURITY_STATE_ALIASES[clean] ?? null;
}

export function parseHeadingState(rawHeading: string): {
  title: string;
...
export function parseStateLine(rawText: string): { state: MaturityState | null; declaredState: string } {
...
export function parseStageStateLine(rawText: string): string {
  for (const rawLine of rawText.split(/\r?\n/)) {
    const line = stripInlineMarkup(rawLine.replace(/^\s*[-*+]\s+/, "")).trim();
    if (!line) continue;
...
export function parseStageSegmentTitle(rawHeading: string): { role: "current" | "next" | "other"; title: string } {
  const heading = stripInlineMarkup(rawHeading).trim();
...
export function parseStageGateLine(rawLine: string): { title: string; state: string } {
  const line = stripInlineMarkup(rawLine).replace(/^\s*[-*+]\s+/, "").trim();
...
export function parseFrontierRole(rawHeading: string): {
  title: string;
  isPrimary: boolean;
  isCoPrimary: boolean;
} {
...
export function parsePostureRole(title: string, rawText: string): PostureRole {
...
export function extractLabeledValue(rawText: string, labels: string[]): string {
...
export function parseArrowTransition(value: string): { before: string; after: string } | null {
...
export function subsectionText(subsections: SemanticSubsection[], labels: string[]): string {
  return (
...
export function isStageBlockerText(rawText: string): boolean {
...
export interface RelationGrammarRule {
  kind: SemanticRelationKind;
  pattern: RegExp;
}

export const RELATION_GRAMMAR_RULES: RelationGrammarRule[] = [
  {
    kind: "area",
...
export function splitRelationTargets(value: string): string[] {
  return value
...
export function isFrontierSubsectionAlias(subheading: string): boolean {
  return FRONTIER_SUBSECTION_ALIASES.some((labels) =>
    labels.some((label) => normalizeKey(subheading).includes(normalizeKey(label)))
  );
}
